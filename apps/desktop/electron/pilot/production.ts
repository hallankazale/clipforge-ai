...
export function decideProductionAfterQualityGate(input: {
  ...
  if (!Number.isInteger(input.currentAttempt) || input.currentAttempt < 1 || ![1, 2, 3].includes(input.maxRetries)) {
    throw new Error('Contagem de tentativas inválida.');
  }
...
export function buildSevenDaySchedule(videosPerDay: 1 | 2 | 3): ScheduleSlot[] {
  ...
  if (!times) throw new Error('Frequência inválida.');
